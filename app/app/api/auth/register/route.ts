...
export async function POST(req: NextRequest) {
  const { firstName, lastName, email, password } = await req.json<{ firstName?: string; lastName?: string; email?: string; password?: string }>();

  if (!firstName?.trim() || !lastName?.trim() || !email || !password || password.length < 8) {
    return NextResponse.json(
      { error: "Enter your first name, last name, email, and a password of at least 8 characters." },
      { status: 400 }
    );
  }
  ...
  await DB.prepare(
    "INSERT INTO users (id, first_name, last_name, email, password_hash, password_salt, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)"
  )
    .bind(userId, firstName.trim(), lastName.trim(), email, hash, salt, now())
    .run();
  ...
  const res = NextResponse.json({ id: userId, firstName: firstName.trim(), lastName: lastName.trim(), email });
